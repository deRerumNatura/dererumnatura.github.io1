$(document).ready(function () {
   ...
   $('#openDescSymbol1').click(function () {
      if (!$('#desc1').is(':visible')) {
         ...
         $('.service-desc-card:nth-child(1)').css('background-image', 'url(../img/_src/services-page/services-desc-pattern-01-01.svg)');
      }
   });
   ...
   if ($(window).width() <= 1024) {
      if ($(window).height() <= 960 && $(window).width() <= 1024) {
         ...
         $('.services-header').css({ 'max-height': '50vh' });
      }
   }
   ...
   if ($('html').width() >= 768) {
      ...
      $('html').bind('mousewheel', function (e) {
         let navPadding = ($('html').width() - $('.container').width()) / 2 + 'px';
